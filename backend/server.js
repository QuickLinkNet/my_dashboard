...
app.get('/api/prompts/pending', function (req, res) {
    db.query('SELECT * FROM prompts WHERE successful_runs < expected_runs LIMIT 5', function (err, results) {
        if (err) {
            return res.status(500).send(err.message);
        }
        res.json(results);
...
app.put('/api/prompts/:id/increment-success', function (req, res) {
    var id = req.params.id;
    // Zuerst den aktuellen Wert von successful_runs abrufen
    db.query('SELECT successful_runs FROM prompts WHERE id = ?', [id], function (err, results) {
        if (err) {
        ...
        var currentSuccessfulRuns = results[0].successful_runs;
        // successful_runs um 1 erhöhen
        var newSuccessfulRuns = currentSuccessfulRuns + 1;
        // Aktualisiere den neuen Wert in der Datenbank
        db.query('UPDATE prompts SET successful_runs = ? WHERE id = ?', [newSuccessfulRuns, id], function (updateErr, updateResult) {
            if (updateErr) {
                return res.status(500).send('Fehler beim Aktualisieren der Daten: ' + updateErr.message);
            }
            res.json({ id: id, successful_runs: newSuccessfulRuns });
        });
    });
});
